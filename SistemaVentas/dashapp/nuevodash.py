...
df["costo"] = df["Product Price"]
df["producto"] = df["Item Name"]
...
def update_dailytrans(start_date,end_date):
    ...
    fig = px.scatter(title="Reg. Transactions vs. Income", x=dfi['costo'], y=dfj['costo'], opacity=0.65, labels=
...
def update_pie(start_date,end_date):
    dff = filter_data(df, start_date, end_date)
    table_df = get_table(dff)
    maxoth = (table_df['Num Transacciones'].max()*15/100)
    table_df = table_df.loc[table_df['Num Transacciones'] >= maxoth]
    traces = []
